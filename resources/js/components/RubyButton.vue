<template>
    ...
    <popover ref="popover" placement="bottom-start" @closed="popoverClosed" :clickaway="false">
...
<script>
import RubyPanel from './RubyPanel.vue';
...
export default {
    ...
    methods: {
        ...
        open() {
            ...
            if (this.panelActive) {
                this.markAttrs = this.editor.getAttributes('ruby');
            } else {
        ...
        update(attributes) {
            this.editor.chain().focus().toggleRuby(attributes).run();
            this.markAttrs = null;
            this.close();
        },
